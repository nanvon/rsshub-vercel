...
:::
`,d={path:`/latest-ranked/:routeParams?`,categories:[`game`],example:`/osu/latest-ranked/includeMode=osu&difficultyLimit=L3&difficultyLimit=U7`,features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1,supportRadar:!0},parameters:{routeParams:{description:`Used to pass route parameters in Query String format. Check out route description for more info.`,default:`null`}},name:`Latest Ranked Beatmap`,description:u,maintainers:[`nfnfgo`],radar:[{source:[`osu.ppy.sh/beatmapsets`]}],handler:f};async function f(t){let l=t.req.param(`routeParams`),u=new URL(`https://osu.ppy.sh?${l}`).searchParams,d=u.getAll(`includeMode`),f=u.getAll(`difficultyLimit`),p=u.get(`modeInTitle`)??`true`,m=await r.tryGet(`https://osu.ppy.sh/beatmapsets:JSON`,async()=>{let e=`https://osu.ppy.sh/beatmapsets`,t=await o.get(e),n=c(t.data),r=JSON.parse(n(`#json-beatmaps`).text()??`{"beatmapsets": undefined}`),i=r.beatmapsets;if(i===void 0)throw Error(`Failed to retrieve JSON beatmap info from osu! website`);return i},n.cache.routeExpire,!1);for(let e of m)e.beatmaps.sort((e,t)=>e.difficulty_rating-t.difficulty_rating);d?.length&&d?.length>0&&(m=m.filter(e=>d.includes(e.beatmaps[0].mode)));let h=99,g=0;if(f&&f.length>0&&f.length<2){for(let e of f)e.startsWith(`U`)?h=Number.parseFloat(e.substring(1)):e.startsWith(`L`)&&(g=Number.parseFloat(e.substring(1)));let e=e=>!(e.beatmaps.at(0).difficulty_rating>h||e.beatmaps.at(-1).difficulty_rating<g);m=m.filter(t=>e(t))}function _(){if(!l)return``;let e=`Feed Configurations:
`;return e+=`Game Mode: ${d.length>0?JSON.stringify(d):`All modes`}\n`,e+=`Star Rating Limit: Lower=${g}, Upper=${h}`,e}let v=m.map(t=>{let n=a(t.ranked_date),r=t.covers[`cover@2x`]||t.covers.cover,o=t.covers[`card@2x`]||t.covers.card,c=`${Math.floor(t.beatmaps[0].total_length/60).toString().padStart(2,`0`)}:${(t.beatmaps[0].total_length%60).toString().padStart(2,`0`)}`,l={osu:`Osu!`,fruits:`Osu!Catch`,taiko:`Osu!Taiko`,mania:`Osu!Mania`},u=i(s.join(e,`templates/beatmapset-d6071e15.art`),{...t,readableTotalLength:c,modeLiteralToDisplayNameMap:l});return{title:`${p===`true`?`[${l[t.beatmaps[0].mode]}] `:``}${t.title_unicode??t.title}`,description:u,pubDate:n,link:`https://osu.ppy.sh/beatmapsets/${t.id}`,category:[`osu!`,`game`],author:[{name:t.creator}],image:r,banner:o,updated:t.last_updated}});return{title:`Osu! Latest Ranked Map`,link:`https://osu.ppy.sh/beatmapsets`,description:`Newly ranked beatmaps at https://osu.ppy.sh/beatmapsets.\n${_()}`,item:v}}export{d as route};