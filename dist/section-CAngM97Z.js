...
| 最新回复 | 最新发布 |
| -------- | -------- |
|          | post     |`};async function s(o){let s=o.req.param(`id`)??``,c=o.req.param(`type`)??`hot`,l=o.req.param(`order`)??``,u=o.req.query(`limit`)?Number.parseInt(o.req.query(`limit`)):10,d=`${r}${s?isNaN(s)?`/category/${s}`:`/section/${s}`:``}`,f=`${n}/api${s?isNaN(s)?`/tags/${s}/`:`/forums/${s}/`:``}threads?type=${c}&includes=tags,forum_name,summary&ps=${u}&pg=1&order=${l===``?``:`time_desc`}&is_groupid=1`;return{title:`一亩三分地 - ${Object.hasOwn(a,s)?a[s]:s}${i[c]}`,link:d,item:await t(e.tryGet,f,l)}}export{o as route};