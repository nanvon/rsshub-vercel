...
| 四川党建               | 1014766 |
| 健康四川               | 1014767 |
| 技能四川               | 12023   |`};async function l(t){let c=t.req.param(`id`)??`1`,l=t.req.param(`limit`)?Number.parseInt(t.req.param(`limit`)):15,u=/t|y/i.test(t.req.param(`isFull`)??`true`),d=`https://www.sctv.com`,f=`https://kscgc.sctv-tf.com`,p=`${f}/sctv/lookback/${c}/date.json`,m=`${f}/sctv/lookback/index/lookbackList.json`,h=`${d}/column/detail?programmeIndex=/sctv/lookback/${c}/index.json`,g=await a({method:`get`,url:p}),_=[],v=g.data.data.programmeArray.slice(0,l).map(e=>({guid:e.id,link:`${f}${e.programmeListUrl}`}));await Promise.all(v.map(t=>n.tryGet(t.link,async()=>{let n=await a({method:`get`,url:t.link}),c=n.data.data.programmeList.map(t=>({guid:t.id,title:t.programmeTitle,link:t.programmeUrl,pubDate:o(i(t.pubTime),8),description:r(s.join(e,`templates/description-9a37f11c.art`),{cover:t.programmeImage,video:t.programmeUrl})})),l=[];u&&(l=c.filter(e=>/（\d{4}(?:\.\d{2}){2}）/.test(e.title))),_=[..._,...l.length===0?c:l]}))),g=await a({method:`get`,url:m});let y,b;for(let e of g.data.data.programme_official)if(e.programmeId===c){y=e.programmeName,b=e.programmeCover;break}return{title:`四川广播电视台 - ${y}`,link:h,item:_.slice(0,t.req.query(`limit`)?Number.parseInt(t.req.query(`limit`)):100),image:b}}export{c as route};