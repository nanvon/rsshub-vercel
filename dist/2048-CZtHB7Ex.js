...
| 聚友客栈 | 坛友自售 |
| -------- | -------- |
| 57       | 136      |`};async function u(t){let l=t.req.param(`id`)??`3`,u=`https://hjd2048.com`,d=await r.tryGet(`2048:domainInfo`,async()=>{let e=await n(`https://2048.info`),t=c(e),r=t(`.button`).first().attr(`onclick`),i=r.match(/window\.open\('([^']+)'/)[1];return{url:i}}),f=await n.raw(d.url),p=`${f.url}thread.php?fid-${l}.html`,m=c(f._data),h=m(`script`).text().match(/var safeid='(.*?)',/)?.[1]??``,g=await n.raw(p,{headers:{cookie:`_safe=${h}`}}),_=c(g._data),v=`https://${new URL(g.url).host}`;_(`#shortcut`).remove(),_(`tr[onmouseover="this.className='tr3 t_two'"]`).remove();let y=_(`#ajaxtable tbody .tr2`).last().nextAll(`.tr3`).toArray().map(e=>(e=_(e).find(`a.subject`),{title:e.text(),link:`${v}/${e.attr(`href`)}`,guid:`${u}/2048/${e.attr(`href`)}`})).filter(e=>!e.link.includes(`undefined`)),b=await Promise.all(y.map(t=>r.tryGet(t.guid,async()=>{let r=await n(t.link,{headers:{cookie:`_safe=${h}`}}),l=c(r);l(`.ads, .tips`).remove(),l(`ignore_js_op`).each(function(){let e=l(this).find(`img`),t=e.attr(`data-original`),n=e.attr(`src`),r=t||n;l(this).replaceWith(`<img src="${r}">`)}),t.author=l(`.fl.black`).first().text(),t.pubDate=o(a(l(`span.fl.gray`).first().attr(`title`)),8);let u=l(`#read_tpc`).first().find(`a`).last(),d=l(`#copytext`)?.first()?.text();if(u?.text()?.startsWith(`http`)&&/bt\.azvmw\.com$/.test(new URL(u.text()).hostname)){let r=await n(u.text()),a=c(r);t.enclosure_type=`application/x-bittorrent`;let o=a(`.uk-button`).last().attr(`href`);t.enclosure_url=o?.startsWith(`http`)?o:`https://bt.azvmw.com/${o}`;let l=a(`.uk-button`).first().attr(`href`);u.replaceWith(i(s.join(e,`templates/download-6b447c6c.art`),{magnet:l,torrent:t.enclosure_url}))}else d?.startsWith(`magnet`)&&(t.enclosure_url=d,t.enclosure_type=`x-scheme-handler/magnet`);let f=l(`#read_tpc`).first();return l(`.showhide img`).each(function(){f.append(`<br><img style="max-width: 100%;" src="${l(this).attr(`src`)}">`)}),t.description=f.html(),t})));return{title:`${_(`#main #breadCrumb a`).last().text()} - 2048核基地`,link:p,item:b}}export{l as route};