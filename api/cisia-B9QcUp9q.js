import"./esm-shims-CtP6w_ML.js";import"./config-DYqAlsU3.js";import"./logger-BlLSmUdl.js";import"./ofetch-CWQqZcqz.js";import"./helpers-RrXnNmv1.js";import{cache_default as e}from"./cache-CvppK6AM.js";import{parseDate as t}from"./parse-date-DHsdom8D.js";import{got_default as n}from"./got-BwctkUCD.js";import{load as r}from"cheerio";const i=async i=>{let{id:a=`9`}=i.req.param(),o=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):20,s=`www.cisia.org`,c=`http://${s}`,l=new URL(`site/term/${a}.html`,c).href,{data:u}=await n(l),d=r(u),f=d(`ul.list_first li`).slice(0,o).toArray().map(e=>{e=d(e);let n=e.find(`a`);return{title:n.text(),pubDate:t(e.find(`span.time`).text()),link:new URL(n.prop(`href`),c).href}});f=await Promise.all(f.map(i=>e.tryGet(i.link,async()=>{if(!/^https?:\/\/www\.cisia\.org(\/[^\s]*)?$/.test(i.link))return i;let{data:e}=await n(i.link),a=r(e),o=a(`div.TextTitle`).text(),s=a(`div.NewsText`).html(),c=a(`div.shar`).text().match(/(\d{4}-\d{2}-\d{2})/)?.[1];return i.title=o,i.description=s,i.pubDate=c?t(c):i.pubDate,i.author=a(`meta[name="Description"]`).prop(`content`),i.content={html:s,text:a(`div.NewsText`).text()},i})));let p=new URL(d(`div.logo img`).prop(`src`),c).href;return{title:d(`title`).text(),description:d(`meta[name="Description"]`).prop(`content`),link:l,item:f,allowEmpty:!0,image:p,author:d(`meta[name="Keywords"]`).prop(`content`)}},a={path:`/:id?`,name:`栏目`,url:`www.cisia.org`,maintainers:[`nczitzk`],handler:i,example:`/cisia/9`,parameters:{id:"栏目 id，默认为 `9`，即协会动态，可在对应分类页 URL 中找到"},description:`::: tip
