import{__dirname as e,init_esm_shims as t}from"./esm-shims-CtP6w_ML.js";import"./config-DYqAlsU3.js";import"./logger-BlLSmUdl.js";import{ofetch_default as n}from"./ofetch-CDAeBYLA.js";import{cache_default as r}from"./cache-7MGQxbL-.js";import{art as i}from"./render-DlHIMz-Z.js";import{parseDate as a}from"./parse-date-B2cCVoGk.js";import{ViewType as o}from"./types-DygLf1a3.js";import s from"node:path";import c from"node:crypto";const l=`query GetProfileProjectsAndSections($username: String, $after: String) {
  ...
  fragment profileSectionFields on ProfileSection {
    ...
    projectCount
    userId
  }`,u=`query GetAppreciatedProjects($username: String, $after: String) {
    user(username: $username) {
  ...
  }`,d=`query ProjectPage($projectId: ProjectId!, $projectPassword: String) {
    viewer {
      ...Project_Viewer
    }
  ...
  }`;t();const f={path:`/:user/:type?`,categories:[`design`,`popular`],view:o.Pictures,example:`/behance/mishapetrick`,parameters:{user:`username`,type:{description:`type`,options:[{value:`projects`,label:`projects`},{value:`appreciated`,label:`appreciated`}],default:`projects`}},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},name:`User Works`,maintainers:[`MisteryMonster`],handler:m,description:"Behance user's profile URL, like [https://www.behance.net/mishapetrick](https://www.behance.net/mishapetrick) the username will be `mishapetrick`。"},p=async(e,t)=>await r.tryGet(`behance:profile:${t}`,()=>{let n=e.flatMap(e=>e.owners).find(e=>e.username===t);return Promise.resolve({displayName:n.displayName,id:n.id,link:n.url,image:n.images.size_50.url.replace(`/user/50/`,`/user/source/`)})});async function m(t){let{user:o,type:f=`projects`}=t.req.param(),m=c.randomUUID(),h={Cookie:`gk_suid=${Math.random().toString().substring(2,10)}, gki=; originalReferrer=; bcp=${m}`,"X-BCP":m,"X-Requested-With":`XMLHttpRequest`},g=await n(`https://www.behance.net/v3/graphql`,{method:`POST`,headers:h,body:{query:f===`projects`?l:u,variables:{username:o,after:``}}}),_=f===`projects`?g.data.user.profileProjects.nodes:g.data.user.appreciatedProjects.nodes,v=_.map(e=>({title:e.name,link:e.url,author:e.owners.map(e=>e.displayName).join(`, `),image:e.covers.size_202.url.replace(`/202/`,`/source/`),pubDate:e.publishedOn?a(e.publishedOn,`X`):void 0,category:e.fields?.map(e=>e.label.toLowerCase()),projectId:e.id})),y=await p(_,o),b=await Promise.all(v.map(t=>r.tryGet(t.link,async()=>{let r=await n(`https://www.behance.net/v3/graphql`,{method:`POST`,headers:h,body:{query:d,variables:{projectId:t.projectId}}}),o=r.data.project;return t.description=i(s.join(e,`templates/description-69dc492c.art`),{description:o.description,modules:o.allModules}),t.category=[...new Set([...t.category||[],...o.tags?.map(e=>e.title.toLowerCase())||[]])],t.pubDate=t.pubDate||(o.publishedOn?a(o.publishedOn,`X`):void 0),t})));return{title:`${y.displayName}'s ${f}`,link:`https://www.behance.net/${o}/${f}`,image:y.image,item:b}}export{f as route};