...
| 乌有公告 | 红色旅游 | 乌有讲堂  | 书画欣赏 |
| -------- | -------- | --------- | -------- |
| gonggao  | lvyou    | jiangtang | shuhua   |`};async function a(i){let a=i.req.param(`id`)??`shidai`,o=`http://www.wyzxwk.com`,s=`${o}/Article/${a}`,c=await n({method:`get`,url:s}),l=r(c.data);l(`.g-sd`).remove();let u=l(`h3 a`).toArray().map(e=>(e=l(e),{title:e.text(),link:e.attr(`href`)}));return u=await Promise.all(u.map(i=>e.tryGet(i.link,async()=>{if(i.link.indexOf(`wyzxwk.com`)>0)try{let e=await n({method:`get`,url:i.link}),a=r(e.data);a(`.zs-modal-body`).prev().nextAll().remove();let o=e.data.match(/<span class="s-grey-3">(\d{4}-\d{2}-\d{2})<\/span>/);o&&(i.pubDate=t(o[1],`YYYY-MM-DD`)),i.description=a(`article`).html()}catch{i.description=``}return i}))),{title:`${l(`title`).text().split(` - `)[0]} - 乌有之乡网刊`,link:s,item:u}}export{i as route};