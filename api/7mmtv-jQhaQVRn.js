...
| ---------- | --------------- | ----------------- | ---------- | ------------------- | ----- | --------- | ------------------ |
| all        | 21              | 30                | 28         | 29                  | 17    | 34        | 33                 |`};async function l(t){let c=t.req.param(`language`)??`en`,l=t.req.param(`category`)??`censored_list`,u=t.req.param(`type`)??`all`,d=`https://7mmtv.sx`,f=`${d}/${c}/${l}/${u}/1.html`,p=await a({method:`get`,url:f}),m=s(p.data),h=m(`.video`).toArray().map(e=>{e=m(e);let t=e.find(`.video-title a`);return{title:t.text(),author:e.find(`.video-channel`).text(),pubDate:i(e.find(`.small`).text()),link:t.attr(`href`),poster:e.find(`img`).attr(`data-src`),video:e.find(`video`).attr(`data-src`)}});return h=await Promise.all(h.map(t=>n.tryGet(t.link,async()=>{let n=await a({method:`get`,url:t.link}),i=s(n.data);return t.description=r(o.join(e,`templates/description-7c12cbba.art`),{cover:i(`.content_main_cover img`).attr(`src`),images:i(`.owl-lazy`).toArray().map(e=>i(e).attr(`data-src`)),description:i(`.video-introduction-images-text`).html(),poster:t.poster,video:t.video}),t.category=i(`.categories a`).toArray().map(e=>i(e).text()),delete t.poster,delete t.video,t}))),{title:m(`title`).text().replace(/ - Watch JAV Online/,``),link:f,item:h,description:m(`meta[name="description"]`).attr(`content`)}}export{c as route};