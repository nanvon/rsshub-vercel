import"./esm-shims-CtP6w_ML.js";import"./config-DYqAlsU3.js";import"./logger-BlLSmUdl.js";import"./ofetch-CWQqZcqz.js";import"./helpers-RrXnNmv1.js";import{parseDate as e}from"./parse-date-DHsdom8D.js";import{got_default as t}from"./got-BwctkUCD.js";import{invalid_parameter_default as n}from"./invalid-parameter-CvwVNf6G.js";import r from"crypto-js";const i={stream:{query:`query UserStreamQuery($lookup: UserLookup!) {
          ...
          }`},uploads:{query:`query UserUploadsQuery(
            $lookup: UserLookup!
          ...
          }`},reposts:{query:`query UserRepostsQuery($lookup: UserLookup!) {
            user: userLookup(lookup: $lookup) {
              username
          ...
          }`},favorites:{query:`query UserFavoritesQuery($lookup: UserLookup!) {
            user: userLookup(lookup: $lookup) {
              username
          ...
          }`},listens:{query:`query UserListensQuery($lookup: UserLookup!) {
            user: userLookup(lookup: $lookup) {
              username
          ...
          }`}},a={path:`/:username/:type?`,categories:[`multimedia`],example:`/mixcloud/dholbach/uploads`,parameters:{username:`Username, can be found in URL`,type:`Type, see below, uploads by default`},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!0,supportScihub:!1},radar:[{source:[`mixcloud.com/:username/:type?`]},{source:[`www.mixcloud.com/:username/:type?`]}],name:`User`,maintainers:[`Misaka13514`],handler:o,description:`| Shows   | Reposts | Favorites | History | Stream |
| ------- | ------- | --------- | ------- | ------ |
| uploads | reposts | favorites | listens | stream |`};async function o(a){let o=`https://www.mixcloud.com`,s=`https://thumbnailer.mixcloud.com/unsafe/480x480/`,c=`https://app.mixcloud.com/graphql`,l={Referer:o,"Content-Type":`application/json`,"X-Requested-With":`XMLHttpRequest`},u=a.req.param(`type`)??`uploads`,d=a.req.param(`username`),f={stream:{name:`Stream`,node:`stream`},uploads:{name:`Shows`,node:`uploads`},reposts:{name:`Reposts`,node:`reposted`},favorites:{name:`Favorites`,node:`favorites`},listens:{name:`History`,node:`listeningHistory`}};if(!f[u])throw new n(`Invalid type: ${u}`);let p={stream:{query:i.stream.query,variables:{lookup:{username:a.req.param(`username`)}}},uploads:{query:i.uploads.query,variables:{lookup:{username:a.req.param(`username`)},orderBy:`LATEST`,onlyAttributedTo:``,hasAttributedTo:!1}},reposts:{query:i.reposts.query,variables:{lookup:{username:a.req.param(`username`)}}},favorites:{query:i.favorites.query,variables:{lookup:{username:a.req.param(`username`)}}},listens:{query:i.listens.query,variables:{lookup:{username:a.req.param(`username`)}}}},m=(await t({method:`post`,url:c,json:p[u],headers:l})).data.data,h=m.user.biog,g=`${s}${m.user.picture.urlRoot}`,_=`IFYOUWANTTHEARTISTSTOGETPAIDDONOTDOWNLOADFROMMIXCLOUD`,v=(e,t)=>{let n=r.enc.Base64.parse(t),i=n.toString(r.enc.Utf8),a=``;for(let t=0;t<i.length;t++)a+=String.fromCharCode(i.charCodeAt(t)^e.charCodeAt(t%e.length));return a},y=m.user[f[u].node].edges.map(t=>{let n=u===`listens`?t.node.cloudcast:t.node;return{title:n.name,author:n.owner.displayName,description:n.description.replaceAll(`
`,`<br>`),pubDate:e(n.publishDate),guid:Buffer.from(n.id,`base64`).toString(`utf8`),link:`${o}/${d}/${n.slug}`,itunes_item_image:`${s}${n.picture.urlRoot}`,itunes_duration:n.audioLength,enclosure_url:v(_,n.streamInfo.url),enclosure_type:`audio/x-m4a`,upvotes:n.favorites.totalCount}});return{title:`Mixcloud - ${m.user.displayName}'s ${f[u].name}`,description:h.replaceAll(`
`,`<br>`),itunes_author:m.user.displayName,image:g,link:`${o}/${d}/${u}/`,item:y}}export{a as route};