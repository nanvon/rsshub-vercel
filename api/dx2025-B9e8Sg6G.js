import"./esm-shims-CtP6w_ML.js";import"./config-DYqAlsU3.js";import"./logger-BlLSmUdl.js";import"./ofetch-CDAeBYLA.js";import"./helpers-RrXnNmv1.js";import{cache_default as e}from"./cache-7MGQxbL-.js";import{got_default as t}from"./got-Cw4D6IS5.js";import{load as n}from"cheerio";const r={path:`/:type?/:category?`,categories:[`new-media`,`popular`],example:`/dx2025`,parameters:{type:`内容类别，见下表，默认为空`,category:`行业分类，见下表，默认为空`},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},name:`分类`,maintainers:[`nczitzk`],handler:i,description:`内容类别
...
| 制造业人才           | manufacturing-talent-industry-reports                             |`};async function i(r){let i=r.req.param(`type`)||``,a=r.req.param(`category`)||``,o=`https://www.dx2025.com`,s=`${o}${i===``?``:`/archives/${i===`tag`?`tag${a===``?``:`/${a}`}`:`category/${i}${a===``?``:`/${a}`}`}`}`,c=await t({method:`get`,url:s}),l=n(c.data),u=l(`.entry-title a`).slice(0,10).map((e,t)=>(t=l(t),{title:t.text(),link:t.attr(`href`)})).get(),d=await Promise.all(u.map(r=>e.tryGet(r.link,async()=>{let e=await t({method:`get`,url:r.link}),i=n(e.data);return r.author=i(`.entry-author-name`).text(),r.description=i(`.bpp-post-content, .entry-content`).html(),r.pubDate=new Date(i(`.entry-date`).attr(`datetime`)).toUTCString(),r})));return{title:l(`title`).text(),link:s,item:d}}export{r as route};