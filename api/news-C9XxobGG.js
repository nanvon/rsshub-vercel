...
`?{tag:`br`,content:null}:e),o=i.extend(e=>({...e,url:e=>({tag:`a`,attrs:{href:Object.keys(e.attrs)[0],rel:`noopener`,target:`_blank`},content:e.content}),video:(e,{render:t})=>({tag:`video`,attrs:{controls:``,preload:`metadata`,poster:e.attrs?.poster},content:t(Object.entries({webm:`video/webm`,mp4:`video/mp4`}).map(([t,n])=>({tag:`source`,attrs:{src:e.attrs?.[t],type:n}})))})})),s=async i=>{let{category:s=`all`,language:c=`english`}=i.req.param(),l=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):100,u=`https://www.counter-strike.net`,d=`https://store.steampowered.com`,f=`https://media.st.dl.eccdnx.com`,p=new URL(`news${s&&s!==`all`?`/${s}`:``}${c?`?l=${c}`:``}`,u).href,m=new URL(`events/ajaxgetpartnereventspageable/`,d).href,{data:h}=await t(m,{searchParams:{clan_accountid:0,appid:730,offset:0,count:l,l:c}}),g=h.events.filter(e=>s===`updates`?e.event_type===12:e.event_type).slice(0,l).map(t=>{let n=t.event_name,i=r(t.announcement_body.body,[o(),a]),s=`counter-strike-news-${t.gid}`;return{title:n,description:i,pubDate:e(t.announcement_body.posttime,`X`),link:new URL(`newsentry/${t.gid}`,u).href,category:t.announcement_body.tags,guid:s,id:s,content:{html:i,text:t.announcement_body.body},updated:e(t.announcement_body.updatetime,`X`)}}),{data:_}=await t(p),v=n(_),y=`Counter Strike`,b=new URL(`apps/csgo/images/dota_react//blog/default_cover.jpg`,f).href;return{title:`${y} - ${s===`updates`?`Updates`:`News`}`,description:v(`title`).text(),link:p,item:g,allowEmpty:!0,image:b,author:y,language:c}},c={path:`/news/:category?/:language?`,name:`News`,url:`www.counter-strike.net`,maintainers:[`nczitzk`],handler:s,example:`/counter-strike/news`,parameters:{category:"Category, `updates` or `all`, `all` by default",language:`Language, english by default, see below for more languages`},description:`::: tip
