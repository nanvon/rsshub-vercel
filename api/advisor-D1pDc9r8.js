...
| swift | Swift | Swift 包管理工具 |`};async function o(a){let{category:o,type:s}=a.req.param(),c=`https://github.com/advisories`,l=`${c}?query=type%3A${s}+ecosystem%3A${o}`,u=`https://github.com/advisories`,d=await r({method:`get`,url:l}),f=i(d.data),p=f(`div.Box-row`).toArray().map(e=>{e=f(e);let t=e.find(`a.Link--primary`),r=e.find(`relative-time`).attr(`datetime`),i=t.text()||`No title`,a=t.attr(`href`)||`#`,o=n(r||``);return{title:i,link:`https://github.com${a}`,pubDate:o,description:``}}),m=await Promise.all(p.map(n=>t.tryGet(n.link,async()=>{let t=await e(n.link),r=i(t);return n.description=r(`.comment-body`).first().html()||``,n})));return{title:`GitHub Advisory Database RSS - ${o} - ${s}`,link:u,item:m}}export{a as route};