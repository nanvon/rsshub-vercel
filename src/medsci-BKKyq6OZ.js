...
| 糖尿病 | 冠心病 | 肝病 | 乳腺癌 |
| ------ | ------ | ---- | ------ |
| 8      | 43     | 22   | 89     |`};async function o(a){let o=a.req.param(`sid`)??``,s=a.req.param(`tid`)??``;o=o===`recommend`?``:o;let c=`https://www.medsci.cn`,l=`${c}${o?`/department/details?s_id=${o}&module=article${s?`&t_id=${s}`:``}`:``}`,u=await r({method:`get`,url:l}),d=i(u.data),f=d(`#articleList`).find(`.ms-link`).toArray().map(n=>{n=d(n);let r=n.parent().parent().find(`.item-meta-item`).first().text();return{title:n.text(),link:`${c}${n.attr(`href`).replace(/;jsessionid=[\dA-Z]+/,``)}`,pubDate:r.indexOf(`-`)>0?e(r):t(r)}});return f=await Promise.all(f.map(t=>n.tryGet(t.link,async()=>{let n=await r({method:`get`,url:t.link}),a=i(n.data),o=n.data.match(/"publishedTime":"(.*)","publishedTimeString"/);return t.author=a(`.name`).text(),t.description=a(`#content`).html(),t.pubDate=o?e(o[1]):t.pubDate,t.category=a(`meta[name="keywords"]`).attr(`content`)?.split(/,|，/).filter(e=>e!==``&&e!==`undefined`)??[],t}))),{title:`${o?d(`.department-header-active`).text():`推荐`} -${s?` ${d(`.department-keywords-ul .active`).text()} -`:``} MedSci.cn`,link:l,item:f}}export{a as route};