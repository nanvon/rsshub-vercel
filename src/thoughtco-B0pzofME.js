...
</details>`};async function l(t){let{category:c=``}=t.req.param(),l=t.req.query(`limit`)?Number.parseInt(t.req.query(`limit`),10):50,u=`https://www.thoughtco.com`,d=new URL(c,u).href,{data:f}=await a(d),p=s(f),m=p(`a[data-doc-id]`).slice(0,l).toArray().map(e=>(e=p(e),{title:e.find(`span.block-title`).text(),link:new URL(e.prop(`href`),u).href}));m=await Promise.all(m.map(t=>r.tryGet(t.link,async()=>{let{data:r}=await a(t.link),c=s(r);return c(`div.adslot`).remove(),c(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),c(`div.figure-media`).each((t,n)=>{n=p(n);let r=n.find(`img`);n.replaceWith(i(o.join(e,`templates/description-87496755.art`),{image:{src:r.prop(`data-src`),width:r.prop(`width`),height:r.prop(`height`)}}))}),t.title=c(`meta[property="og:title"]`).prop(`content`),t.description=i(o.join(e,`templates/description-87496755.art`),{image:{src:c(`meta[property="og:image"]`).prop(`content`)},description:c(`div.article-content`).html()}),t.author=c(`meta[name="sailthru.author"]`).prop(`content`),t.category=[...new Set(c(`meta[name="parsely-tags"]`).prop(`content`)?.split(/,/).map(e=>e.trim()))],t.pubDate=n(r.match(/"datePublished": "(.*?)"/)[1]),t.updated=n(r.match(/"dateModified": "(.*?)"/)[1]),t})));let h=p(`meta[property="og:site_name"]`).prop(`content`),g=p(`meta[property="og:title"]`).prop(`content`),_=new URL(p(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`),u).href;return{item:m,title:`${h}${g.startsWith(h)?``:` - ${g}`}`,link:d,description:p(`meta[property="og:description"]`).prop(`content`),language:p(`html`).prop(`lang`),image:p(`meta[property="og:image"]`).prop(`content`),icon:_,logo:_,subtitle:p(`meta[property="og:title"]`).prop(`content`),author:h}}export{c as route};