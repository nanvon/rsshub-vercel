...
| 聚友客栈 | 坛友自售 |
| -------- | -------- |
| 57       | 136      |`};async function u(t){let l=t.req.param(`id`)??`3`,u=await r.tryGet(`2048:domainInfo`,async()=>({url:c(await n(`https://2048.info`))(`.button`).first().attr(`onclick`).match(/window\.open\('([^']+)'/)[1]})),d=await n.raw(u.url),f=`${d.url}thread.php?fid-${l}.html`,p=c(d._data)(`script`).text().match(/var safeid='(.*?)',/)?.[1]??``,m=await n.raw(f,{headers:{cookie:`_safe=${p}`}}),h=c(m._data),g=`https://${new URL(m.url).host}`;h(`#shortcut`).remove(),h(`tr[onmouseover="this.className='tr3 t_two'"]`).remove();let _=h(`#ajaxtable tbody .tr2`).last().nextAll(`.tr3`).toArray().map(e=>(e=h(e).find(`a.subject`),{title:e.text(),link:`${g}/${e.attr(`href`)}`,guid:`https://hjd2048.com/2048/${e.attr(`href`)}`})).filter(e=>!e.link.includes(`undefined`)),v=await Promise.all(_.map(t=>r.tryGet(t.guid,async()=>{let r=c(await n(t.link,{headers:{cookie:`_safe=${p}`}}));r(`.ads, .tips`).remove(),r(`ignore_js_op`).each(function(){let e=r(this).find(`img`),t=e.attr(`data-original`),n=e.attr(`src`),i=t||n;r(this).replaceWith(`<img src="${i}">`)}),t.author=r(`.fl.black`).first().text(),t.pubDate=o(a(r(`span.fl.gray`).first().attr(`title`)),8);let l=r(`#read_tpc`).first().find(`a`).last(),u=r(`#copytext`)?.first()?.text();if(l?.text()?.startsWith(`http`)&&/bt\.azvmw\.com$/.test(new URL(l.text()).hostname)){let r=c(await n(l.text()));t.enclosure_type=`application/x-bittorrent`;let a=r(`.uk-button`).last().attr(`href`);t.enclosure_url=a?.startsWith(`http`)?a:`https://bt.azvmw.com/${a}`;let o=r(`.uk-button`).first().attr(`href`);l.replaceWith(i(s.join(e,`templates/download-6b447c6c.art`),{magnet:o,torrent:t.enclosure_url}))}else u?.startsWith(`magnet`)&&(t.enclosure_url=u,t.enclosure_type=`x-scheme-handler/magnet`);let d=r(`#read_tpc`).first();return r(`.showhide img`).each(function(){d.append(`<br><img style="max-width: 100%;" src="${r(this).attr(`src`)}">`)}),t.description=d.html(),t})));return{title:`${h(`#main #breadCrumb a`).last().text()} - 2048核基地`,link:f,item:v}}export{l as route};