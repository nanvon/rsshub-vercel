...
| ✅学位授予-规章制度    | xwsy_gzzd    |
| ✅学位授予-授位名单    | xwsy_swmd    |
| ✅学位授予-资料下载    | xwsy_zlxz    |`,radar:[{source:[`gr.xidian.edu.cn/`]}]};async function s(o){let{category:s=`home_tzgg1`}=o.req.param(),c=`${i}/${a[s].path}.htm`,l=await n(c,{headers:{referer:i},https:{rejectUnauthorized:!1}}),u=r(l.data);if(s===`yyjs_jbqk`||s===`yyjs_jbqk1`||s===`yyjs_jbqk2`||s===`yyjs_jbqk4`)return{title:u(`.right-bt-left`).text(),link:c,item:[{title:u(`.right-bt-left`).text(),link:c,description:u(`.main-right`).html()}]};{let o=u(a[s].selector.list).toArray().map(t=>(t=u(t),{title:t.find(`a`).text(),link:new URL(t.find(`a`).attr(`href`),i).href,pubDate:e(t.find(`span`).text())}));return o=await Promise.all(o.map(e=>t.tryGet(e.link,async()=>{let t=await n(e.link,{headers:{referer:c},https:{rejectUnauthorized:!1}}),i=r(t.data);return i(`.content-sxt`).remove(),e.description=i(`[name="_newscontent_fromname"]`).html(),e}))),{title:u(`title`).text(),link:c,item:o}}}export{o as route};