import"./esm-shims-Dqvxr0BZ.js";import"./config-BpwDbAkH.js";import"./logger-B3QfaIfn.js";import{ofetch_default as e}from"./ofetch-TsvCL7Kz.js";import{cache_default as t}from"./cache-CzcusEyX.js";import{parseDate as n}from"./parse-date-DHsdom8D.js";import{ViewType as r}from"./types-Bi82qguM.js";import{load as i}from"cheerio";const a=async r=>{let{category:a=`ywgg/tzgg`}=r.req.param(),o=Number.parseInt(r.req.query(`limit`)??`15`,10),s=`https://www.cccmc.org.cn`,c=new URL(a.endsWith(`/`)?a:`${a}/`,s).href,l=await e(c),u=i(l),d=u(`html`).attr(`lang`)??`zh-CN`,f=[],p=/\{url:'(.*)',title:'(.*)',time:'(.*)'\},/g;f=l.match(p)?.slice(0,o).map(e=>{let t=e.match(/'(.*?)'/),r=t?.[2]??``,i=t?.[3],a=t?.[1],o=i,c={title:r,pubDate:i?n(i):void 0,link:a?new URL(a,s).href:void 0,updated:o?n(o):void 0,language:d};return c})??[],f=(await Promise.all(f.map(r=>r.link?t.tryGet(r.link,async()=>{let t=await e(r.link),a=i(t),o=a(`div.title`).text(),s=a(`div#article-content`).html()??``,c=a(`span.time`).text().split(/：/).pop(),l=a(`span.form, span.from`).toArray(),u=l.map(e=>{let t=a(e);return{name:t.text().split(/：/).pop()??t.text()}}),f=c,p={title:o,description:s,pubDate:c?n(c):r.pubDate,author:u,content:{html:s,text:s},updated:f?n(f):r.updated,language:d};return{...r,...p}}):r))).filter(e=>!0);let m=u(`title`).text();return{title:m,description:m.split(/-/)[0].trim(),link:c,item:f,allowEmpty:!0,image:u(`img.logo`).attr(`src`),author:m.split(/-/)?.pop()?.trim(),language:d,id:c}},o={path:`/:category{.+}?`,name:`通用`,url:`www.cccmc.org.cn`,maintainers:[`nczitzk`],handler:a,example:`/cccmc/ywgg/tzgg`,parameters:{category:"分类，默认为 `ywgg/tzgg`，即通知公告，可在对应分类页 URL 中找到, Category, `ywgg/tzgg`，即通知公告  by default"},description:`:::tip
