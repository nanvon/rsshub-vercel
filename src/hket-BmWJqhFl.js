...
</details>`};async function d(t){let{category:u=`sran001`}=t.req.param(),d=l[u.slice(0,4)].baseUrl,f=await n(`${d}/${u}`),p=c.load(f),m=p(`.main-listing-container div.listing-title > a`).toArray().map(e=>{e=p(e);let t=e.parent().parent().find(`.share-button`).data(`url`);return{title:e.text().trim(),link:t.startsWith(`http`)?t:d+t}}),h=await Promise.all(m.map(t=>r.tryGet(t.link,async()=>{if(t.link.startsWith(`https://invest.hket.com/`)||t.link.startsWith(`https://ps.hket.com/`)){let e=await(t.link.startsWith(`https://invest.hket.com/`)?n(`https://invest.hket.com/content-api-middleware/content`,{headers:{referer:t.link},method:`POST`,body:{id:t.link.split(`/`).pop(),channel:`invest`}}):n(`https://data02.hket.com/content`,{headers:{referer:t.link},query:{id:t.link.split(`/`).pop(),channel:`epc`}}));return t.pubDate=o(a(e.displayDate),8),t.updated=o(a(e.lastModifiedDate),8),t.author=e.authors?.map(e=>e.name).join(`, `),t.description=e.content.full||e.content.partial,t.category=e.contentTags?.map(e=>e.name),t}let r=await n(t.link),l=c.load(r);t.category=l(`.contentTags-container > .hotkey-container-wrapper > .hotkey-container > a`).toArray().map(e=>l(e).text().trim()),l(`source`).remove(),l(`p.article-detail_caption, .article-extend-button, span.click-to-enlarge`).remove(),l(`.loyalty-promotion-container, .relatedContents-container, .article-details-center-sharing-btn, .article-detail_login`).remove(),l(`.gallery-related-container, .contentTags-container`).remove(),l(`.listing-widget-126, div.template-default.hket-row.no-padding.detail-widget`).remove(),l(`.ad_MobileMain, .adunit, .native-ad`).remove(),l(`span`).each((e,t)=>{l(t).text().startsWith(`+`)&&l(t).remove()}),l(`img`).each((t,n)=>{n=l(n),n.replaceWith(i(s.join(e,`templates/image-5e0e542d.art`),{alt:n.data(`alt`),src:n.data(`src`)??n.attr(`src`)}))});let u=JSON.parse(l(`script[type="application/ld+json"]`).toArray().find(e=>l(e).text().includes(`NewsArticle`))?.children[0].data);return t.description=l(`div.article-detail-body-container`).html(),t.pubDate=a(u.datePublished),t.updated=a(u.dateModified),t})));return{title:p(`head meta[name=title]`).attr(`content`)?.trim(),link:d+`/`+u,description:p(`head meta[name=description]`).attr(`content`)?.trim(),item:h,language:`zh-hk`}}export{u as route};