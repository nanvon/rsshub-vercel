...
</details>`,handler:c};async function c(a){let{category:s=`xwzx`}=a.req.param(),c=a.req.query(`limit`)?Number.parseInt(a.req.query(`limit`),10):30,l=`https://www.cs.com.cn`,u=new URL(s.endsWith(`/`)?s:`${s}/`,l).href,{data:d}=await n(u,{responseType:`buffer`}),f=i(o(d)),p=f(`ul.ch_type3_list li a`).slice(0,c).toArray().map(t=>(t=f(t),{title:t.find(`h3`).text().trim(),link:new URL(t.prop(`href`),u).href,pubDate:r(e(t.find(`em`).text()),8)}));p=await Promise.all(p.map(a=>t.tryGet(a.link,async()=>{try{let{data:t}=await n(a.link,{responseType:`buffer`}),s=i(o(t));a.title=s(`article.cont_article header h1`).text().trim(),a.description=s(`article.cont_article section`).html(),a.author=s(`div.artc_info em`).text().trim(),a.category=s(`div.artc_route div a`).slice(1).toArray().map(e=>s(e).prop(`title`)??s(e).text()),a.pubDate=r(e(s(`.time`).prop(`datetime`)),8)}catch{}return a})));let m=f(`title`).text(),h=new URL(f(`div.logo_cs a img`).prop(`src`),u).href,g=new URL(`favicon.ico`,l).href;return{item:p,title:m,link:u,description:f(`meta[name="Description"]`).prop(`content`),language:f(`html`).prop(`lang`),image:h,icon:g,logo:g,subtitle:f(`meta[name="Keywords"]`).prop(`content`),author:m.split(`-`).pop().trim()}}export{s as route};