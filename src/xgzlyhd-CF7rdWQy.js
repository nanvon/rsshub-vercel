...
| 离退办                       | 6bf265ffd1c94fa4a3f1687b03fa908b |`};async function h(t){let{category:l,department:d}=t.req.param(),f=t.req.query(`limit`)?Number.parseInt(t.req.query(`limit`),10):10,m,h;l&&(m=await p(`category`,l)),d&&(h=await p(`department`,d));let{data:g}=await i.post(c,{json:{clyj:``,curPage:`1`,endTime:``,fzsj:h?.code??``,lybt:void 0,lylx:m?.code??``,lynr:``,startTime:``,zj:``}}),_=g.data.data.slice(0,f).map(t=>({title:t.lybt,link:`${u}#${t.zj}`,description:r(a.join(e,`templates/description-5c2bbb00.art`),{item:t}),author:`${t.lyr} ⇄ ${t.fzsjCn}`,category:[t.fzsjCn],guid:`${u}#${t.zj}`,pubDate:n(t.pubtime)})),{data:v}=await i(u),y=o(v),b=`国家市场监督管理总局`,x=y(`title`).text(),S=[m?m.name:void 0,h?h.name:void 0].filter(Boolean).join(` - `),C=new URL(y(`link[rel="icon"]`).prop(`href`),s).href;return{item:_,title:`${b}${x}${S?` - ${S}`:``}`,link:u,description:y(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:new URL(`gjjly/${y(`div.fd-logo img`).prop(`src`)}`,s).href,icon:C,logo:C,subtitle:S,author:b,allowEmpty:!0}}export{m as route};