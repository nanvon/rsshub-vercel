...
| lxkc     |
  ...
  工作通知：无`};async function s(o){let s=o.req.param(`author`)||`gztz`,c=o.req.param(`category`)||`all`,l=i+a[s][c]+`/list.htm`,u=await n(l,{headers:{Referer:i}}),d=r(u.data),f=d(`.wp_article_list_table .border9`).toArray().map(t=>(t=d(t),{title:t.find(`a`).attr(`title`),link:new URL(t.find(`a`).attr(`href`),i).href,pubDate:e(t.find(`.date`).text())})),p=await Promise.all(f.map(e=>t.tryGet(e.link,async()=>{if(e.link.includes(`.htm`)){let t=await n(e.link),i=r(t.data);e.description=i(`.wp_articlecontent`).html().trim()}else e.description=`此链接为文件，请点击下载`;return e})));return{title:`哈尔滨工程大学本科生院工作通知`,link:l,item:p}}export{o as route};