...
| 更新时间 | 豆瓣评分 |
| -------- | -------- |
| 0        | 1        |`};async function m(t){let p=t.req.param(`caty`)||`all`,m=t.req.param(`type`)||`all`,h=t.req.param(`area`)||`all`,g=t.req.param(`year`)||`all`,_=t.req.param(`order`)||`0`,v=t.req.query(`domain`)||`bdys01.com`;if(!n.feature.allow_user_supply_unsafe_domain&&!f.has(new URL(`https://${v}`).hostname))throw new c(`This RSS is disabled unless 'ALLOW_USER_SUPPLY_UNSAFE_DOMAIN' is set to 'true'.`);let y=`https://www.${v}`,b=`${y}/s/${p}?${m===`all`?``:`&type=`+m}${h===`all`?``:`&area=`+h}${g===`all`?``:`&year=`+g}&order=${_}`,x=await o({method:`get`,url:b}),S=u(x.data),C=``,w=S(`.card-body .card a`).slice(0,15).toArray().map(e=>{e=S(e);let t=e.attr(`href`).split(`;jsessionid=`);C=t[1];let n=e.next();return{title:n.find(`h3`).text(),link:`${y}${t[0]}`,pubDate:r(n.find(`.text-muted`).text())}}),T={cookie:`JSESSIONID=${C}`},E=await d(w,t=>i.tryGet(t.link,async()=>{let n=await o({method:`get`,url:t.link,headers:T}),i=await o({method:`get`,url:`${y}/downloadInfo/list?mid=${t.link.split(`/`)[4].split(`.`)[0]}`,headers:T}),c=u(n.data);c(`svg`).remove();let d=c(`.download-list .list-group`);return t.description=a(l.join(e,`templates/desc-7474f0b1.art`),{info:c(`.row.mt-3`).html(),synopsis:c(`#synopsis`).html(),links:i.data,torrents:d.html()}),t.pubDate=s(r(c(`.bg-purple-lt`).text().replace(`更新时间：`,``)),8),t.guid=`${t.link}#${c(`.card h1`).text()}`,t.enclosure_url=d.html()?`${y}${d.find(`a`).first().attr(`href`)}`:i.data.pop().url,t.enclosure_type=`application/x-bittorrent`,t}),{concurrency:1});return{title:`哔嘀影视`,link:b,item:E}}export{p as route};