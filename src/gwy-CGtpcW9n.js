...
| 浙江省台州市 | 13310 |
| 浙江省丽水市 | 13311 |
| 省级单位     | 13317 |`};async function a(i){let{category:a,column:o}=i.req.param(),s=i.req.query(`limit`)?Number.parseInt(i.req.query(`limit`),10):50,c=`http://gwy.zjks.gov.cn`,l=new URL(`zjgwy/website/${a?`queryMore`:`init`}.htm`,c).href,u=new URL(`zjgwy/website/queryDetail.htm`,c).href,{data:d}=await n.post(l,{form:{dsdm:o,mkxh:a,oldornew:`new`}}),f=r(d),p=f(`a[onclick^="queryDetail"]`).slice(0,s).toArray().map(t=>{t=f(t);let n=t.prop(`onclick`).match(/queryDetail\('?(\d+)'?, '?(\d+)'?\);/);return{title:t.text(),link:u,category:n[1],guid:`zjks-${n[1]}-${n[2]}`,pubDate:e(t.parent().next().text()),tzid:n[2]}});p=await Promise.all(p.map(e=>t.tryGet(e.guid,async()=>{let{data:t}=await n.post(u,{form:{mkxh:e.category,oldornew:`new`,dsdm:o,tzid:e.tzid}}),i=r(t);e.description=i(`div.ibox-content`).last().html(),e.category=[i(`div.ibox-title`).last().find(`h5`).first().text()];let a=i(`a.ke-insertfile`);if(a.length>0){let t=a.first();e.enclosure_url=t.prop(`href`)}return delete e.tzid,e})));let m=f(`button.btn-success`).last().text(),h=f(`table`).parent().prev().find(`h5`).text();return{item:p,title:`${f(`title`).text()} - ${m}${h}`,link:l,description:f(`div.title-font2`).text(),subtitle:`${m}${h}`,author:f(`div.title-font`).text(),allowEmpty:!0}}export{i as route};