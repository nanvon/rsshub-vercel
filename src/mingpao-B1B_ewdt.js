...
| s00016   | 娛樂         |
| s00017   | English      |
| s00018   | 作家專欄     |`};async function p(e){let t=e.req.param(`type`)??`ins`,i=e.req.param(`category`)??(t===`ins`?`all`:`s00001`),s=`https://news.mingpao.com/rss/${t}/${i}.xml`,l=await o.parseURL(s),f=await Promise.all(l.items.map(e=>r.tryGet(e.link,async()=>{let t=await a(e.link,{headers:{Referer:`https://news.mingpao.com/`}}),r=c.load(t),i=r(`#topvideo`).length?r(`#topvideo iframe`).toArray().map(e=>r(e).attr(`href`,r(e).attr(`src`))).map(e=>d(e,r)):[],o=r(`a.fancybox`).length?r(`a.fancybox`):r(`a.fancybox-buttons`);r(`div.ad300ins_m`).remove(),r(`div.clear, div.inReadLrecGroup, div.clr`).remove(),r(`div#ssm2`).remove(),r(`iframe`).remove(),r(`p[dir=ltr]`).remove(),e.category=e.categories;let s=[...i,...o.toArray().map(e=>d(e,r))],l=r(`script`).toArray().find(e=>r(e).text()?.includes(`$('#lower').prepend('`)),f=l?r(l).text()?.match(/\$\('#lower'\)\.prepend\('(.*)'\);/)?.[1]?.replaceAll(/\\"/g,`"`):``;if(f){let e=c.load(f,null,!1);s=[...s,...e(`a.fancybox`).toArray().map(t=>d(t,e))]}return delete e.categories,delete e.content,delete e.contentSnippet,delete e.creator,delete e.isoDate,e.description=u(s,r(`.txt4`).html()??r(`.article_content.line_1_5em`).html()??r(`.txt3`).html()),e.pubDate=n(e.pubDate),e.guid=e.link.includes(`?`)?e.link:e.link.slice(0,e.link.lastIndexOf(`/`)),e})));return{title:l.title,link:l.link,description:l.description,item:f,image:l.image.url,language:l.language}}export{f as route};