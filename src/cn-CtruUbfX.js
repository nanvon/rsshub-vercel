...
| 交通运输与物流                  | 旅游、运输和物流   |
| 出海与国际化、转型              | 出海与国际化、转型 |
| 全球基础材料                    | 全球基础材料       |`};async function s(r){let{category:o=`25`}=r.req.param(),s=``;if(o!==`25`)if(a[o])s=a[o].slug;else{let e=Object.values(a).find(e=>e.slug===o);s=e?e.slug:``}let c=`https://www.mckinsey.com.cn/insights/${s?`${s}/`:``}`,l={"accept-language":`en-US,en;q=0.9`,"sec-fetch-dest":`document`,"sec-fetch-mode":`navigate`,"sec-fetch-site":`none`},u=await e(c,{headers:l}),d=i(u),f=d(`.fl-post-grid-post`).toArray().map(e=>{let t=d(e),r=t.find(`h2 a`);return{title:r.attr(`title`),description:t.find(`.fl-post-grid-content`).html()?.trim(),link:r.attr(`href`),pubDate:t.find(`[itemprop="datePublished"]`).length?n(t.find(`[itemprop="datePublished"]`).attr(`content`)):void 0}}),p=await Promise.all(f.map(n=>t.tryGet(n.link,async()=>{let t=await e(n.link,{headers:l}),r=i(t);return n.description=r(`.ast-post-format-`).html()||n.description,n.guid=r(`link[rel="shortlink"]`).attr(`href`),n})));return{title:d(`head title`).text(),link:c,image:d(`meta[name="msapplication-TileImage"]`).attr(`content`),item:p}}export{o as route};