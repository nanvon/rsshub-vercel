...
</details>`};async function l(e){let{category:c=``}=e.req.param(),l=e.req.query(`limit`)?Number.parseInt(e.req.query(`limit`),10):50,u=`https://www.thoughtco.com`,d=new URL(c,u).href,{data:f}=await a(d),p=s(f),m=p(`a[data-doc-id]`).slice(0,l).toArray().map(e=>(e=p(e),{title:e.find(`span.block-title`).text(),link:new URL(e.prop(`href`),u).href}));m=await Promise.all(m.map(e=>n.tryGet(e.link,async()=>{let{data:n}=await a(e.link),c=s(n);return c(`div.adslot`).remove(),c(`div.sources-and-citation, .mntl-figure-caption svg`).remove(),c(`div.figure-media`).each((e,n)=>{n=p(n);let r=n.find(`img`);n.replaceWith(i(o.join(t,`templates/description-87496755.art`),{image:{src:r.prop(`data-src`),width:r.prop(`width`),height:r.prop(`height`)}}))}),e.title=c(`meta[property="og:title"]`).prop(`content`),e.description=i(o.join(t,`templates/description-87496755.art`),{image:{src:c(`meta[property="og:image"]`).prop(`content`)},description:c(`div.article-content`).html()}),e.author=c(`meta[name="sailthru.author"]`).prop(`content`),e.category=[...new Set(c(`meta[name="parsely-tags"]`).prop(`content`)?.split(/,/).map(e=>e.trim()))],e.pubDate=r(n.match(/"datePublished": "(.*?)"/)[1]),e.updated=r(n.match(/"dateModified": "(.*?)"/)[1]),e})));let h=p(`meta[property="og:site_name"]`).prop(`content`),g=p(`meta[property="og:title"]`).prop(`content`),_=new URL(p(`link[rel="apple-touch-icon-precomposed"]`).prop(`href`),u).href;return{item:m,title:`${h}${g.startsWith(h)?``:` - ${g}`}`,link:d,description:p(`meta[property="og:description"]`).prop(`content`),language:p(`html`).prop(`lang`),image:p(`meta[property="og:image"]`).prop(`content`),icon:_,logo:_,subtitle:p(`meta[property="og:title"]`).prop(`content`),author:h}}export{c as route};