...
| 更新时间 | 豆瓣评分 |
| -------- | -------- |
| 0        | 1        |`};async function m(t){let p=t.req.param(`caty`)||`all`,m=t.req.param(`type`)||`all`,h=t.req.param(`area`)||`all`,g=t.req.param(`year`)||`all`,_=t.req.param(`order`)||`0`,v=t.req.query(`domain`)||`bdys01.com`;if(!n.feature.allow_user_supply_unsafe_domain&&!f.has(new URL(`https://${v}`).hostname))throw new c(`This RSS is disabled unless 'ALLOW_USER_SUPPLY_UNSAFE_DOMAIN' is set to 'true'.`);let y=`https://www.${v}`,b=`${y}/s/${p}?${m===`all`?``:`&type=`+m}${h===`all`?``:`&area=`+h}${g===`all`?``:`&year=`+g}&order=${_}`,x=u((await o({method:`get`,url:b})).data),S=``,C=x(`.card-body .card a`).slice(0,15).toArray().map(e=>{e=x(e);let t=e.attr(`href`).split(`;jsessionid=`);S=t[1];let n=e.next();return{title:n.find(`h3`).text(),link:`${y}${t[0]}`,pubDate:a(n.find(`.text-muted`).text())}}),w={cookie:`JSESSIONID=${S}`};return{title:`哔嘀影视`,link:b,item:await d(C,t=>r.tryGet(t.link,async()=>{let n=await o({method:`get`,url:t.link,headers:w}),r=await o({method:`get`,url:`${y}/downloadInfo/list?mid=${t.link.split(`/`)[4].split(`.`)[0]}`,headers:w}),c=u(n.data);c(`svg`).remove();let d=c(`.download-list .list-group`);return t.description=i(l.join(e,`templates/desc-7474f0b1.art`),{info:c(`.row.mt-3`).html(),synopsis:c(`#synopsis`).html(),links:r.data,torrents:d.html()}),t.pubDate=s(a(c(`.bg-purple-lt`).text().replace(`更新时间：`,``)),8),t.guid=`${t.link}#${c(`.card h1`).text()}`,t.enclosure_url=d.html()?`${y}${d.find(`a`).first().attr(`href`)}`:r.data.pop().url,t.enclosure_type=`application/x-bittorrent`,t}),{concurrency:1})}}export{p as route};