...
fragment ImageControlIcons_Post on Post {
  ...
  favoritesCount
  ...PostComment_Post
}`,variables:{identifier:e,first:t,after:``},operationName:`CreatorPostReelList`}),method:`POST`});return n.data.posts.nodes});t();const d={path:`/r18/creator/:identifier`,categories:[`multimedia`],example:`/fantube/r18/creator/miyuu`,parameters:{identifier:`User handle`},features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1},radar:[{source:[`www.fantube.tokyo/r18/creator/:identifier`]}],name:`User Posts`,maintainers:[`TonyRL`],handler:p},f=({description:t,thumbnailUrl:n,sampleVideoId:r,imageUrls:a})=>i(o.join(e,`templates/post-7ca176ac.art`),{description:t,thumbnailUrl:n,sampleVideoId:r,imageUrls:a});async function p(e){let{identifier:t}=e.req.param(),r=Number.parseInt(e.req.query(`limit`)||18,10),i=await l(t),a=await u(t,r),o=a.map(e=>({title:e.title.replaceAll(`
`,` `).trim(),description:f({description:e.description,thumbnailUrl:e.thumbnailUrl,sampleVideoId:e.sampleVideoId,imageUrls:e.contentData?.imageUrls||[]}),link:`${c}/r18/post/${e.id}?creator=${t}`,author:e.creator.displayName,pubDate:n(e.publishStartAt),image:e.thumbnailUrl}));return{title:`${i.displayName}のプロフィール｜クリエイターページ｜FANTUBE(ファンチューブ)`,link:`${c}/r18/creator/${t}`,description:i.description,image:i.avatarImageUrl,icon:i.avatarImageUrl,logo:i.avatarImageUrl,language:`ja`,item:o}}export{d as route};