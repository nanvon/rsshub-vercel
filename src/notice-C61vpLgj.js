...
| ------------ |
| 3686         |
</details>`};async function s(o){let{channelId:s=2834}=o.req.param(),c=`https://www.chinamoney.com.cn`,{data:l}=await n.post(`${c}/ags/ms/cm-s-notice-query/contents`,{form:{pageNo:1,pageSize:15,channelId:s}}),u=l.records.map(t=>({title:t.title,link:`${c}${t.draftPath}`,pubDate:r(e(t.releaseDate,`YYYY-MM-DD`),8),contentId:t.contentId})),d=await Promise.all(u.map(a=>t.tryGet(a.link,async()=>{let{data:t}=await n(a.link),o=i(t),s=o(`.article-a-body`);return s.find(`*`).removeAttr(`style`),s.find(`font`).each((e,t)=>{o(t).replaceWith(o(t).html())}),s.find(`span`).each((e,t)=>{o(t).replaceWith(o(t).html())}),s.find(`.article-a-attach-body a`).each((e,t)=>{t=o(t),t.attr(`onclick`)?.startsWith(`location.href=encodeURI($('#fileDownUrl').val()+'fileDownLoad.do`)&&(t.attr(`href`,`${c}/dqs/cm-s-notice-query/fileDownLoad.do?mode=open&contentId=${a.contentId}&priority=${e}`),t.removeAttr(`onclick`))}),a.description=s.html(),a.pubDate=r(e(o(`.AC-l span`).text().trim(),`YYYY-MM-DD HH:mm`),8),a})));return{title:`${a[s]?a[s].title+` - `:``}中国货币网`,link:`${c}${a[s]?.urlPath??``}`,item:d}}export{o as route};