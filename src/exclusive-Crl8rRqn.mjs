...
| 今日之声 | jrzs |
| 浪潮     | lc   |
| 沸点     | fd   |`};async function d(e){let u=e.req.param(`id`)??``,d=`https://3g.163.com`,f=`${d}/touch/exclusive${u?`/sub/${u}`:``}`,p=await a({method:`get`,url:`${d}/touch/reconstruct/article/list/${l[u].id}wangning/0-20.html`}),m=JSON.parse(p.data.match(/^artiList\((.*)\)$/)[1])[`${l[u].id}wangning`].map(e=>({title:e.title,author:e.source,link:e.skipURL||e.url||`${d}/dy/article/${e.docid}.html`,pubDate:o(r(e.ptime),8),videoId:e.skipType===`video`?e.stitle:``}));return m=await Promise.all(m.map(e=>n.tryGet(e.link,async()=>{try{if(e.videoId){let n=await a({method:`get`,url:`${d}/touch/video/detail/jsonp/VIA8K0PTB.html?callback=videoList`}),r=JSON.parse(n.data.match(/^videoList\((.*)\)$/)[1])?.mp4_url;e.description=i(s.join(t,`templates/exclusive-d4c229b8.art`),{video:r})}else{let n=c((await a({method:`get`,url:e.link})).data);n(`.m-linkCard`).remove(),n(`.m-photo`).each(function(){n(this).html(i(s.join(t,`templates/exclusive-d4c229b8.art`),{image:n(this).find(`img`).attr(`data-src`)}))}),e.description=n(`.article-body`).html()}}catch{}return delete e.videoId,e}))),{title:`网易独家 - ${l[u].title}`,link:f,item:m}}export{u as route};