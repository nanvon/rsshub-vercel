...
| 简体中文   | zh        |
| 한국어     | ko        |
| Other      | other     |`};async function s(t){let{category:o=`mind-map-examples`,language:s=`en`}=t.req.param(),c=`https://www.mindmeister.com${s===`en`||s===`other`?``:`/${s}`}/${o===`mind-map-examples`?o:`mind-maps/${o}?language=${s}`}`,l=a((await r(c)).data),u=l(`#public-listing .map-tile-wrapper`).toArray().map(t=>{t=l(t);let r=new URL(t.find(`.map-wrapper`).attr(`style`).match(/url\('(.*)'\);/)[1]).href;return{title:t.find(`.title`).text(),description:n(i.join(e,`templates/image-e463f33f.art`),{src:r.split(`?`)[0],alt:t.find(`.title`).text().trim()}),link:t.find(`.title`).attr(`href`),author:t.find(`.author`).text().trim().replace(/^by/,``),category:t.find(`.fw-bold`).text()}});return{title:l(`head title`).text(),description:l(`head meta[name=description]`).text(),link:c,item:u,language:s}}export{o as route};