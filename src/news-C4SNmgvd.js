...
| 教育前沿 | jyqy |
| 文化体育 | whty |
| 最新科技 | zxkj |`};async function o(a){let o=a.req.param(`type`),s=`${i}/${o}/list.htm`,c=await n(s),l=c.data,u=r(l),d=u(`.column-news-list > .news_list > .news`).toArray().map(t=>(t=u(t),{title:t.find(`a`).attr(`title`),link:new URL(t.find(`a`).attr(`href`),i).href,pubDate:e(t.find(`.news_meta`).text(),`YYYY-MM-DD`)})),f=await Promise.all(d.map(e=>t.tryGet(e.link,async()=>{let t=await n(e.link),i=r(t.data);return e.author=i(`.arti-metas`).text().split(`更新日期`)[0],e.description=i(`.article_content`).html(),e})));return{title:`东北大学新闻网-${u(`head title`).text()}`,link:s,item:f}}export{a as route};