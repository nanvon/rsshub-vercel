...
Example:
- \`/bsky/profile/bsky.app/filter=posts_with_replies\``};async function f(t){let a=t.req.param(`handle`),d=u.parse(t.req.param(`routeParams`)),f=d.filter||`posts_and_author_threads`,p=await c(a,r.tryGet),m=await s(p,r.tryGet),h=await o(p,f,r.tryGet),g=h.feed.map(({post:t})=>({title:t.record.text.split(`
`)[0],description:i(l.join(e,`templates/post-7b0043bb.art`),{text:t.record.text.replaceAll(`
`,`<br>`),embed:t.embed}),author:t.author.displayName,pubDate:n(t.record.createdAt),link:`https://bsky.app/profile/${t.author.handle}/post/${t.uri.split(`app.bsky.feed.post/`)[1]}`,upvotes:t.likeCount,comments:t.replyCount}));return t.set(`json`,{DID:p,profile:m,authorFeed:h}),{title:`${m.displayName} (@${m.handle}) — Bluesky`,description:m.description?.replaceAll(`
