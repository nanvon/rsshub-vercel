...
</details>`};async function u(e){let{industry:l,label:u}=e.req.param(),d=e.req.query(`limit`)?Number.parseInt(e.req.query(`limit`),10):50,f=`https://www.questmobile.com.cn`,p=new URL(`api/v2/report/article-list`,f).href,m=new URL(`api/v2/report/industry-label-tree`,f).href,{data:{data:{industryTree:h,labelTree:g}}}=await a(m),_=c(h),v=c(g),y=l?_.find(e=>e.key===l||e.value===l):void 0,b=u?v.find(e=>e.key===u||e.value===u):y?void 0:v.find(e=>e.key===l||e.value===l),x=y?.key??-1,S=b?.key??-1,C=new URL(`research/reports/${y?.key??-1}/${b?.key??-1}`,f).href,{data:w}=await a(p,{searchParams:{version:0,pageSize:d,pageNo:1,industryId:x,labelId:S}}),T=w.data.slice(0,d).map(e=>({title:e.title,link:new URL(`research/report/${e.id}`,f).href,description:r(o.join(t,`templates/description-32f758d3.art`),{image:{src:e.coverImgUrl,alt:e.title},introduction:e.introduction,description:e.content}),category:[...e.industryList??[],...e.labelList??[]],guid:`questmobile-report#${e.id}`,pubDate:i(e.publishTime)}));T=await Promise.all(T.map(e=>n.tryGet(e.link,async()=>{let{data:n}=await a(e.link),c=s(n);return c(`div.text div.daoyu`).remove(),e.title=c(`div.title h1`).text(),e.description+=r(o.join(t,`templates/description-32f758d3.art`),{description:c(`div.text`).html()}),e.author=c(`div.source`).text().replace(/^.*?：/,``),e.category=c(`div.hy, div.keyword`).find(`span`).toArray().map(e=>c(e).text()),e.pubDate=i(c(`div.data span`).prop(`datetime`)),e})));let{data:E}=await a(C),D=s(E),O=D(`meta[property="og:title"]`).prop(`content`).split(/-/)[0],k=[y?.value,b?.value].filter(Boolean),A=D(`img[alt="${O}"]`).prop(`src`),j=D(`link[rel="shortcut icon"]`).prop(`href`);return{item:T,title:`${O}${k.length===0?``:` - ${k.join(` - `)}`}`,link:C,description:D(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:A,icon:j,logo:j,subtitle:D(`meta[name="keywords"]`).prop(`content`),author:O,allowEmpty:!0}}export{l as route};