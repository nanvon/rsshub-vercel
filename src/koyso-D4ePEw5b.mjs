import{n as e,t}from"./esm-shims-BKVRry6h.mjs";import"./config-uQhBoCds.mjs";import"./logger-DQf7gybc.mjs";import{t as n}from"./ofetch-7N8fmlQ6.mjs";import{t as r}from"./cache-BXZZTZVV.mjs";import{t as i}from"./render-_fW2cbJz.mjs";import{t as a}from"./types-CaC5w-Pp.mjs";import o from"node:path";import{load as s}from"cheerio";e();const c=async e=>{let{category:a=`0`,sort:c=`latest`}=e.req.param(),l=Number.parseInt(e.req.query(`limit`)??`30`,10),u=`https://koyso.to`,d=new URL(`?${a===`0`?``:`category=${a}&`}sort=${c}`,u).href,f=s(await n(d)),p=f(`html`).attr(`lang`)??`en`,m=[];m=f(`a.game_item`).slice(0,l).toArray().map(e=>{let n=f(e),r=n.find(`div.game_info`).text(),a=n.find(`div.game_media img`).attr(`data-src`),s=i(o.join(t,`templates/description-463c9506.art`),{images:a?[{src:a,alt:r}]:void 0}),c=n.attr(`href`);return{title:r,description:s,link:c?new URL(c,u).href:void 0,content:{html:s,text:s},image:a,banner:a,language:p}}),m=await Promise.all(m.map(e=>e.link?r.tryGet(e.link,async()=>{let t=s(await n(e.link));t(`div.ind`).remove(),t(`div.download_div`).remove();let r=t(`h1.content_title`).text();t(`h1.content_title`).remove();let i=e.description+(t(`div.game_content`).html()||``),a={title:r,description:i,content:{html:i,text:i},language:p};return{...e,...a}}):e));let h=f(`ul.category li#category_${a}`).text(),g=f(`div.genres_content ul li.${c}`).text();return{title:`${f(`title`).text()} - ${h} - ${g}`,description:f(`meta[name="description"]`).attr(`content`),link:d,item:m,allowEmpty:!0,language:p,id:d}},l={path:`/:category?/:sort?`,name:`游戏`,url:`koyso.to`,maintainers:[`nczitzk`],handler:c,example:`/koyso/0/latest`,parameters:{category:{description:"排序，默认为 `0`，即全部，可在对应分类页 URL 中找到",options:[{label:`全部游戏`,value:`0`},{label:`动作游戏`,value:`3`},{label:`冒险游戏`,value:`5`},{label:`绅士游戏`,value:`7`},{label:`射击游戏`,value:`1`},{label:`休闲游戏`,value:`2`},{label:`体育竞速`,value:`4`},{label:`模拟经营`,value:`6`},{label:`角色扮演`,value:`8`},{label:`策略游戏`,value:`9`},{label:`格斗游戏`,value:`10`},{label:`恐怖游戏`,value:`11`},{label:`即时战略`,value:`12`},{label:`卡牌游戏`,value:`13`},{label:`独立游戏`,value:`14`},{label:`局域网联机`,value:`15`}]},sort:{description:"排序，默认为 `latest`，即最新，可在对应页 URL 中找到",options:[{label:`热度`,value:`views`},{label:`最新`,value:`latest`}]}},description:`::: tip
