...
| -------------- | ------------ | ------------ |
| 1              | 2            | 3            |`};async function s(o){let s=o.req.param(`type`)??`recommend`,c=o.req.param(`time`)??`5`,l=o.req.param(`sort`)??`1`,u=`http://blog.sciencenet.cn`,d=`${u}/blog.php?mod=${s}&type=list&op=${c}&ord=${l}`,f=await n({method:`get`,url:d,responseType:`buffer`}),p=i(a.decode(f.data,`gbk`)),m=p(`tr td a[title]`).slice(0,o.req.query(`limit`)?Number.parseInt(o.req.query(`limit`)):50).toArray().map(e=>(e=p(e),{title:e.text(),link:`${u}/${e.attr(`href`)}`,pubDate:new Date(e.next().text()).toUTCString()}));return m=await Promise.all(m.map(o=>t.tryGet(o.link,async()=>{let t=await n({method:`get`,url:o.link,responseType:`buffer`}),s=i(a.decode(t.data,`gbk`));return o.author=s(`.xs2`).text(),o.description=s(`#blog_article`).html(),o.pubDate=r(e(s(`.xg1`).eq(5).text()),8),o}))),{title:`科学网 - 精选博文`,link:d,item:m}}export{o as route};