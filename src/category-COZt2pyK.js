...
| ---------- | ----- | --------- | ---------------- | ------------ | ------- | ------- | -------- | ----------------------- | ------- | ------ |
| coverstory | first | editorial | current_affairs | editor_desk | economy | finance | business | environment_technology | cwcivil | column |`};async function l(c){let l=c.req.param(`category`),u=c.req.param(`column`),d=`https://${u}.caixin.com/${l}`;if(!a(u))throw new i(`Invalid column`);let f=await n(d),p=s(f.data),m=p(`head title`).text(),h=JSON.parse(p(`script`).text().match(/var entity = ({.*?})/)[1]),{data:{datas:g}}=await n(`https://gateway.caixin.com/api/extapi/homeInterface.jsp`,{searchParams:{subject:h.id,type:0,count:c.req.query(`limit`)?Number.parseInt(c.req.query(`limit`)):25,picdim:`_266_177`,start:0}}),_=g.map(t=>({title:t.desc,description:t.summ,link:t.link.replace(`http://`,`https://`),pubDate:r(e(t.time),8),category:t.keyword.split(` `),audio:t.audioUrl,audio_image_url:t.pict.imgs[0].url})),v=await Promise.all(_.map(e=>t.tryGet(e.link,()=>o(e))));return{title:m,link:d,description:`财新网 - 提供财经新闻及资讯服务`,item:v}}export{c as route};