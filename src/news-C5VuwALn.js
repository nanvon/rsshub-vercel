...
`}};async function c(s){let c=[`en`,`EN`,`us`,`US`,`www`,`WWW`,``].includes(s.req.param(`region`))?``:s.req.param(`region`).toLowerCase(),l=s.req.param(`category`);if(![`hk`,`tw`,`au`,`ca`,`fr`,`malaysia`,`nz`,`sg`,`uk`,``].includes(c))throw new t(`Unknown region: ${c}`);let u=s.req.query(`limit`)?Number.parseInt(s.req.query(`limit`),10):20;if([`hk`,`tw`].includes(c)){let t=await i(c,e.tryGet),n=o(c,await r(c,u,l?t[l].yctMap:null)),s=await Promise.all(n.map(t=>a(t,e.tryGet)));return{title:`Yahoo 新聞 ${c.toUpperCase()} - ${l?t[l].name:`所有類別`}`,link:`https://${c}.news.yahoo.com/${l?`${l}/`:``}archive`,image:`https://s.yimg.com/cv/apiv2/social/images/yahoo_default_logo-1200x1200.png`,item:s}}else{let t=`https://${c?`${c}.`:``}news.yahoo.com/rss/${l?`${l}/`:``}`,r=await n.parseURL(t),i=r.items.filter(e=>e?.link&&!e.link.includes(`promotions`)&&new URL(e.link).hostname.match(/.*\.yahoo\.com$/)),o=await Promise.all(i.map(t=>a(t,e.tryGet)));return{title:`Yahoo News ${c.toUpperCase()} - ${l?l.toUpperCase():`All`}`,link:r.link,description:r.description,item:o}}}export{s as route};