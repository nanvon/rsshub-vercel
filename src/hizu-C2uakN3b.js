...
| 高新     | 607d37ade4b05c59ac2f3d40 |`};async function o(a){let o=a.req.param(`column`)??`5dd92265e4b0bf88dd8c1175`,s=`https://www.hizh.cn`,c=`${s}/channels/zjyapp/columns/${o}/stories.json`,l=(await n({method:`get`,url:c})).data.stories.slice(0,a.req.query(`limit`)?Number.parseInt(a.req.query(`limit`)):50).map(e=>({title:e.title,author:e.author,link:e.jsonUrl,category:e.columnName,pubDate:t(e.published)}));return l=await Promise.all(l.map(t=>e.tryGet(t.link,async()=>{let e=await n({method:`get`,url:t.link}),i=r(e.data.content);return i(`figure`).last().remove(),t.description=i.html(),t.link=`${s}/content.html?jsonUrl=${t.link}`,t}))),{title:`${i[o]} - 珠海网`,link:s,item:l}}export{a as route};