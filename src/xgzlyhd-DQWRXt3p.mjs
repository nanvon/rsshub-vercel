...
| 离退办                       | 6bf265ffd1c94fa4a3f1687b03fa908b |`};async function h(e){let{category:l,department:d}=e.req.param(),f=e.req.query(`limit`)?Number.parseInt(e.req.query(`limit`),10):10,m,h;l&&(m=await p(`category`,l)),d&&(h=await p(`department`,d));let{data:g}=await i.post(c,{json:{clyj:``,curPage:`1`,endTime:``,fzsj:h?.code??``,lybt:void 0,lylx:m?.code??``,lynr:``,startTime:``,zj:``}}),_=g.data.data.slice(0,f).map(e=>({title:e.lybt,link:`${u}#${e.zj}`,description:r(a.join(t,`templates/description-5c2bbb00.art`),{item:e}),author:`${e.lyr} ⇄ ${e.fzsjCn}`,category:[e.fzsjCn],guid:`${u}#${e.zj}`,pubDate:n(e.pubtime)})),{data:v}=await i(u),y=o(v),b=`国家市场监督管理总局`,x=y(`title`).text(),S=[m?m.name:void 0,h?h.name:void 0].filter(Boolean).join(` - `),C=new URL(y(`link[rel="icon"]`).prop(`href`),s).href;return{item:_,title:`${b}${x}${S?` - ${S}`:``}`,link:u,description:y(`meta[property="og:description"]`).prop(`content`),language:`zh`,image:new URL(`gjjly/${y(`div.fd-logo img`).prop(`src`)}`,s).href,icon:C,logo:C,subtitle:S,author:b,allowEmpty:!0}}export{m as route};