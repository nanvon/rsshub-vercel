...
| 广州   | 371  |
| 大湾区 | 397  |
| 城区   | 2980 |`};async function u(e){let l=e.req.param(`column`)??74,{data:u}=await a(`https://app.gzdaily.cn/app_if/getArticles?columnId=${l}&page=1`),d=u.list.filter(e=>e.newstype===0).map(e=>({title:e.title,description:i(s.join(t,`templates/description-60b509f9.art`),{thumb:e.picBig}),pubDate:o(r(e.publishtime),8),link:e.shareUrl,colName:e.colName,author:e.arthorName})),f=``,p=await Promise.all(d.map(e=>n.tryGet(e.link,async()=>{let t=c((await a({method:`get`,url:e.link})).data);return f=f===``?e.colName:f,t(`.abstract`).text()&&(t(`.abstract`).find(`span`).remove(),e.description+=`<blockquote>`+t(`.abstract`).text()+`</blockquote>`),e.description+=t(`.article`).html()??``,e})));return{title:`广州日报客户端 - ${f}`,link:`https://www.gzdaily.cn/amucsite/web/index.html#/home/${l}`,item:p}}export{l as route};