...
| 临安  | 富阳   | 桐庐   | 建德   | 淳安   |
| ----- | ------ | ------ | ------ | ------ |
| linan | fuyang | tonglu | jiande | chunan |`};async function u(l){let u=l.req.param(`city`)??`www`;if(!a(u))throw new i(`Invalid city`);let d=`https://${u}.19lou.com`,f=await n({method:`get`,url:d,responseType:`buffer`}),p=o(s.decode(f.data,`gbk`));p(`.title-more`).remove();let m=p(`.center-center-jiazi`).find(`a[title]`).toArray().map(e=>(e=p(e),{title:e.attr(`title`),link:`https:${e.attr(`href`)}`}));return m=await Promise.all(m.map(i=>t.tryGet(i.link,async()=>{let t=await n({method:`get`,url:i.link,responseType:`buffer`,headers:{cookie:c(`_Z3nY0d4C_`,`37XgPK9h`,365,`/`,`19lou.com`),referer:d}}),a=o(s.decode(t.data,`gbk`));return a(`.name-lz, .postView-pk-mod`).remove(),i.author=a(`.uname, .user-name`).first().text(),i.description=a(`.post-cont`).first().html()||a(`.thread-cont`).html(),i.pubDate=r(e(a(`.cont-top-left meta`).first().attr(`content`)),8),i}))),{title:p(`title`).text().split(`-`)[0],link:d,item:m}}export{l as route};