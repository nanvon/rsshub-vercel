...
| 交通运输与物流                  | 旅游、运输和物流   |
| 出海与国际化、转型              | 出海与国际化、转型 |
| 全球基础材料                    | 全球基础材料       |`};async function s(r){let{category:o=`25`}=r.req.param(),s=``;if(o!==`25`)if(a[o])s=a[o].slug;else{let e=Object.values(a).find(e=>e.slug===o);s=e?e.slug:``}let c=`https://www.mckinsey.com.cn/insights/${s?`${s}/`:``}`,l={"accept-language":`en-US,en;q=0.9`,"sec-fetch-dest":`document`,"sec-fetch-mode":`navigate`,"sec-fetch-site":`none`},u=await n(c,{headers:l}),d=i(u),f=d(`.fl-post-grid-post`).toArray().map(t=>{let n=d(t),r=n.find(`h2 a`);return{title:r.attr(`title`),description:n.find(`.fl-post-grid-content`).html()?.trim(),link:r.attr(`href`),pubDate:n.find(`[itemprop="datePublished"]`).length?e(n.find(`[itemprop="datePublished"]`).attr(`content`)):void 0}}),p=await Promise.all(f.map(e=>t.tryGet(e.link,async()=>{let t=await n(e.link,{headers:l}),r=i(t);return e.description=r(`.ast-post-format-`).html()||e.description,e.guid=r(`link[rel="shortlink"]`).attr(`href`),e})));return{title:d(`head title`).text(),link:c,image:d(`meta[name="msapplication-TileImage"]`).attr(`content`),item:p}}export{o as route};