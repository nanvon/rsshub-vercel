import"./esm-shims-B6VgfGYP.js";import"./config-CVBRPN4O.js";import"./logger-BvonkID1.js";import{ofetch_default as e}from"./ofetch-CHIwZNTj.js";import"./render-CSnicFA3.js";import"./parse-date-DHsdom8D.js";import"./timezone-CMz5pnRe.js";import{buildApiUrl as t,processItems as n,rootUrl as r}from"./util-Cezyw4SX.js";import{load as i}from"cheerio";const a=async a=>{let{id:o}=a.req.param(),[s,c]=o?.split(/-/)??[void 0,void 0],l=Number.parseInt(a.req.query(`limit`)??`30`,10),u=new URL(`discover${o?`/${o}`:``}`,r).href,d=await e(u),f=i(d),{apiRecommListUrl:p,apiRecommProcUrl:m,apiTagProcUrl:h}=await t(f),g,_,v=!!(s&&c);if(v){let t=((await e(p))?.data?.results??[]).find(e=>String(e.Id)===s),n=t?t.sublist.find(e=>String(e.Id)===c):void 0;g=t?.tag??t?.alias??void 0,_=n?.tag??n?.alias??void 0,v=!!(g&&_)}let y={page:1,pagesize:l,ticket:``},{data:{results:b}}=await(v?e(m,{query:{...y,ptag:g,stag:_}}):e(h,{query:{...y,f:`id`,o:`desc`}})),x=n(b?.slice(0,l)??[]),S=new URL(f(`img.logo`).prop(`src`),r).href,C=f(`title`).text().split(/_/).pop();return{title:`${C}${v?` | ${g} - ${_}`:``}`,description:f(`meta[property="og:description"]`).prop(`content`),link:u,item:x,allowEmpty:!0,image:S,author:C}},o={path:`/discover/:id?`,name:`发现`,url:`top.aibase.com`,maintainers:[`nczitzk`],handler:a,example:`/aibase/discover`,parameters:{id:`发现分类，默认为空，即全部产品，可在对应发现分类页 URL 中找到`},description:`::: tip
