...
| 乌有公告 | 红色旅游 | 乌有讲堂  | 书画欣赏 |
| -------- | -------- | --------- | -------- |
| gonggao  | lvyou    | jiangtang | shuhua   |`};async function a(i){let a=i.req.param(`id`)??`shidai`,o=`http://www.wyzxwk.com/Article/${a}`,s=await n({method:`get`,url:o}),c=r(s.data);c(`.g-sd`).remove();let l=c(`h3 a`).toArray().map(e=>(e=c(e),{title:e.text(),link:e.attr(`href`)}));return l=await Promise.all(l.map(i=>t.tryGet(i.link,async()=>{if(i.link.indexOf(`wyzxwk.com`)>0)try{let t=await n({method:`get`,url:i.link}),a=r(t.data);a(`.zs-modal-body`).prev().nextAll().remove();let o=t.data.match(/<span class="s-grey-3">(\d{4}-\d{2}-\d{2})<\/span>/);o&&(i.pubDate=e(o[1],`YYYY-MM-DD`)),i.description=a(`article`).html()}catch{i.description=``}return i}))),{title:`${c(`title`).text().split(` - `)[0]} - 乌有之乡网刊`,link:o,item:l}}export{i as route};