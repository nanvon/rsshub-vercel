...
| -------- | ------ |
| 最新主题 | latest |
| 精华主题 | digest |`};async function c(s){let c=s.req.param(`category`)||`all`,l=s.req.param(`type`)||`latest`,u,d;Object.hasOwn(o,c)?l===`digest`?(u=`forum-${o[c][0]}-1.htm?digest=1`,d=`看雪论坛精华主题 - ${o[c][1]}`):(u=`forum-${o[c][0]}.html`,d=`看雪论坛最新主题 - ${o[c][1]}`):c===`digest`?(u=`new-digest.htm`,d=`看雪论坛精华主题`):(u=`new-tid.htm`,d=`看雪论坛最新主题`);let f=i((await r({method:`get`,url:a+u,headers:{Referer:a}})).data),p=f(`.thread`),m=await Promise.all(p?p.toArray().filter(e=>{let r=f(`.date`,e).eq(0).text(),i=r.endsWith(`前`)?n(r):t(r.slice(1));return!e.attribs.class.includes(`top`)||Date.now()-i.valueOf()<2592e5}).map(o=>{let s=f(`.subject a`,o).eq(1),c=f(`.date`,o).eq(0).text(),l=c.endsWith(`前`)?n(c):t(c.slice(1)),u=`${a}${s.attr(`href`)}`,d=`kanxue: ${u}`;return e.tryGet(d,async()=>{let e=i((await r({method:`get`,url:u})).data);e(`.card`).eq(0).find(`.message img`).each((t,n)=>{n=e(n);let r=n.attr(`src`);r!==void 0&&!r.startsWith(`https://`)&&!r.startsWith(`http://`)&&n.attr(`src`,`https://bbs.kanxue.com/${r}`)});let t=e(`.card`).eq(0).find(`.message`).html();return{title:s.text(),link:u,pubDate:l,description:t}})}):[]);return{title:d,link:a+u,item:m,allowEmpty:!0}}export{s as route};