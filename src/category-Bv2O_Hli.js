...
}`}}),u=l.data.posts.map(e=>({title:e.title,pubDate:t(e.publishedDate),category:[...(e.sections??[]).map(e=>`section:${e.name}`),...(e.categories??[]).map(e=>`category:${e.name}`)],link:`${c}/story/${e.slug}`})),d=await Promise.all(u.map(e=>n(e)));return{title:`鏡週刊 Mirror Media - ${i}`,link:c,item:d}}export{r as route};