...
| 所有 | 绘画 | 视频 | 写作 | 游戏 | 音乐 | 播客 | 摄影 | 技术 | Vtuber | 舞蹈 | 体育 | 旅游 | 美食 | 时尚 | 数码 | 动画 | 其他 |`,handler:i};async function i(r){let{type:i=`rec`,category:a=`所有`}=r.req.param(),o=`https://afdian.com`,s=`${o}/api/creator/list`,c=(await e(s,{searchParams:{type:i,category_id:t[a]}})).data.data.list.map(e=>{let{doing:t,monthly_fans:n,detail:r}=e.creator;return{title:e.name,description:`正在创作 ${t}<br/>
            ...
            详情:<br/>
            ${r}<br/>
            `,link:`${o}/@${e.url_slug}`}});return{title:`爱发电-创作者 (按 ${a}/${n[i]})`,description:`爱发电-发现创作者 (按 ${a}/${n[i]})`,link:`${o}/explore`,item:c}}export{r as route};