...
##### \`f_TPR\`
  ...
  **To make it easier, the recommended way is to start a search on [LinkedIn](https://www.linkedin.com/jobs/search) and use [RSSHub Radar](https://github.com/DIYgod/RSSHub-Radar) to load the specific feed.**`};async function p(n){let d=l(n.req.param(`job_types`),a),f=l(n.req.param(`exp_levels`),r),p=new URLSearchParams(n.req.param(`routeParams`)),m=new URL(`/jobs-guest/jobs/api/seeMoreJobPostings/search`,`https://www.linkedin.com/`);m.searchParams.append(s,n.req.param(`keywords`)||``),m.searchParams.append(o,d),m.searchParams.append(i,f);for(let[e,t]of p)m.searchParams.has(e)||m.searchParams.append(e,t);m=m.toString();let h=await e(m),g=c(h),_=u(n.req.param(`job_types`),a),v=u(n.req.param(`exp_levels`),r);return{title:`LinkedIn Job Listing`+(_?` | Job Types: ${_}`:``)+(v?` | Experience Levels: ${v}`:``)+(n.req.param(`keywords`)?` | Keywords: ${n.req.param(`keywords`)}`:``),link:m,description:`This feed gets LinkedIn job posts`,item:g.map(e=>{let n=`${e.company} is hiring ${e.title}`,r=`Title: ${e.title} | Company: ${e.company} | Location: ${e.location} `;return{title:n,description:r,pubDate:t(e.pubDate),link:e.link}})}}export{f as route};