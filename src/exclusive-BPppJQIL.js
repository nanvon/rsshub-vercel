...
| 今日之声 | jrzs |
| 浪潮     | lc   |
| 沸点     | fd   |`};async function d(t){let u=t.req.param(`id`)??``,d=`https://3g.163.com`,f=`${d}/touch/exclusive${u?`/sub/${u}`:``}`,p=await a({method:`get`,url:`${d}/touch/reconstruct/article/list/${l[u].id}wangning/0-20.html`}),m=JSON.parse(p.data.match(/^artiList\((.*)\)$/)[1])[`${l[u].id}wangning`].map(e=>({title:e.title,author:e.source,link:e.skipURL||e.url||`${d}/dy/article/${e.docid}.html`,pubDate:o(i(e.ptime),8),videoId:e.skipType===`video`?e.stitle:``}));return m=await Promise.all(m.map(t=>n.tryGet(t.link,async()=>{try{if(t.videoId){let n=await a({method:`get`,url:`${d}/touch/video/detail/jsonp/VIA8K0PTB.html?callback=videoList`}),i=JSON.parse(n.data.match(/^videoList\((.*)\)$/)[1])?.mp4_url;t.description=r(s.join(e,`templates/exclusive-d4c229b8.art`),{video:i})}else{let n=c((await a({method:`get`,url:t.link})).data);n(`.m-linkCard`).remove(),n(`.m-photo`).each(function(){n(this).html(r(s.join(e,`templates/exclusive-d4c229b8.art`),{image:n(this).find(`img`).attr(`data-src`)}))}),t.description=n(`.article-body`).html()}}catch{}return delete t.videoId,t}))),{title:`网易独家 - ${l[u].title}`,link:f,item:m}}export{u as route};