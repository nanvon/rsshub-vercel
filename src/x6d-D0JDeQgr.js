...
| -------- | -------- | -------- | -------- | -------- | -------- | -------- |
| 106      | 107      | 108      | 109      | 110      | 111      | 113      |`,categories:[`new-media`],features:{requireConfig:!1,requirePuppeteer:!1,antiCrawler:!1,supportBT:!1,supportPodcast:!1,supportScihub:!1}};async function o(a){let{id:o=`latest`}=a.req.param(),s=a.req.query(`limit`)?Number.parseInt(a.req.query(`limit`),10):22,c=`https://xd.x6d.com`,l=new URL(o===`latest`?``:`html/${o}.html`,c).href,{data:u}=await n(l),d;if(/<meta\s/.test(u))d=i(u);else{l=new URL(o===`latest`?``:u.match(/'([\w./=?]+)'/g).toReversed().join(``).replaceAll(`'`,``),c).href;let{data:e}=await n(l);d=i(e)}d(`i.rj`).remove();let f=o===`latest`?d(`#newslist ul`).first().find(`li`).not(`li.addd`).find(`a`).slice(0,s):d(`a.soft-title`).slice(0,s),p=f.toArray().map(e=>(e=d(e),{title:e.prop(`title`)??e.text(),link:new URL(e.prop(`href`),c).href,language:`zh`}));p=await Promise.all(p.map(a=>t.tryGet(a.link,async()=>{let{data:t}=await n(a.link),o=i(t),s=o(`h1.article-title`).text(),l=o(`div.article-content`).html(),u=new URL(o(`div.article-content img`).first().prop(`src`),c).href;return a.title=s,a.description=l,a.pubDate=r(e(o(`time`).text()),8),a.category=o(`b.bq-wg`).toArray().map(e=>o(e).text()),a.author=o(`span.bq-zz`).text(),a.content={html:l,text:o(`div.article-content`).text()},a.image=u,a.banner=u,a.language=`zh`,a})));let m=new URL(d(`div.header-logo img`).prop(`src`),c).href;return{title:d(`title`).text().split(/\s-/)[0],description:d(`meta[name="description"]`).prop(`content`),link:l,item:p,allowEmpty:!0,image:m,language:`zh`}}export{o as handler,a as route};