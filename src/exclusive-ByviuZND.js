...
| 今日之声 | jrzs |
| 浪潮     | lc   |
| 沸点     | fd   |`};async function d(t){let u=t.req.param(`id`)??``,d=`https://3g.163.com`,f=`${d}/touch/exclusive${u?`/sub/${u}`:``}`,p=`${d}/touch/reconstruct/article/list/${l[u].id}wangning/0-20.html`,m=await a({method:`get`,url:p}),h=JSON.parse(m.data.match(/^artiList\((.*)\)$/)[1])[`${l[u].id}wangning`],g=h.map(e=>({title:e.title,author:e.source,link:e.skipURL||e.url||`${d}/dy/article/${e.docid}.html`,pubDate:o(n(e.ptime),8),videoId:e.skipType===`video`?e.stitle:``}));return g=await Promise.all(g.map(t=>r.tryGet(t.link,async()=>{try{if(t.videoId){let n=await a({method:`get`,url:`${d}/touch/video/detail/jsonp/VIA8K0PTB.html?callback=videoList`}),r=JSON.parse(n.data.match(/^videoList\((.*)\)$/)[1])?.mp4_url;t.description=i(s.join(e,`templates/exclusive-d4c229b8.art`),{video:r})}else{let n=await a({method:`get`,url:t.link}),r=c(n.data);r(`.m-linkCard`).remove(),r(`.m-photo`).each(function(){r(this).html(i(s.join(e,`templates/exclusive-d4c229b8.art`),{image:r(this).find(`img`).attr(`data-src`)}))}),t.description=r(`.article-body`).html()}}catch{}return delete t.videoId,t}))),{title:`网易独家 - ${l[u].title}`,link:f,item:g}}export{u as route};