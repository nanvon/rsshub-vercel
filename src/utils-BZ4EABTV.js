import{parseDate as e}from"./parse-date-D4osZfpm.js";const t=(e,t=``)=>{for(let n of e.content)switch(n.type){case`text`:t+=`<div>${n.content}</div>`;break;case`image`:t+=`<img src="${n.url.replaceAll(/_.+\.jpg/g,`.jpg`)}" />`;break;case`video`:try{t+=`<video
            controls="controls"
