import{__dirname as e,init_esm_shims as t}from"./esm-shims-CGUM9TC6.js";import"./config-C9m9eBw5.js";import"./logger-CC3afzWM.js";import{parseDate as n}from"./parse-date-D4osZfpm.js";import"./dist-0j_z2rfc.js";import"./helpers-nWIDkd0K.js";import{cache_default as r}from"./cache-GwMhal7i.js";import{art as i}from"./render-DOz85fGU.js";import"./ofetch-ZUQ1sEMc.js";import{got_default as a}from"./got-CB83B8oA.js";import{timezone as o}from"./timezone-BrNu6iXe.js";import s from"node:path";import{load as c}from"cheerio";t();const l=async t=>{let{category:l=`zx/zxfb/`}=t.req.param(),u=t.req.query(`limit`)?Number.parseInt(t.req.query(`limit`),10):10,d=`http://www.agri.cn`,f=new URL(l.endsWith(`/`)?l:`${l}/`,d).href,{data:p}=await a(f),m=c(p),h=m(`html`).prop(`lang`),g=m(`div.list_li_con, div.nxw_video_com`).slice(0,u).toArray().map(t=>{t=m(t);let r=t.find(`a`).first(),a=r.text(),o=t.find(`img`).first().prop(`src`)?new URL(t.find(`img`).first().prop(`src`),d).href:void 0,c=i(s.join(e,`templates/description-881baa33.art`),{intro:t.find(`p.con_text`).text()||void 0,images:o?[{src:o,alt:a}]:void 0});return{title:a,description:c,pubDate:n(t.find(`span.con_date_span`).text()||`${t.find(`div.com_time_p2`).text().trim()}${t.find(`div.com_time_p1`).text()}`,[`YYYY-MM-DD`,`YYYY.MM.DD`]),link:new URL(r.prop(`href`),f).href,content:{html:c,text:t.find(`p.con_text`).text()||void 0},image:o,banner:o,language:h}});g=await Promise.all(g.map(t=>r.tryGet(t.link,async()=>{let{data:r}=await a(t.link),l=c(r),u=l(`div.detailCon_info_tit`).text().trim(),d=i(s.join(e,`templates/description-881baa33.art`),{description:l(`div.content_body_box`).html()});return t.title=u,t.description=d,t.pubDate=o(n(l(`meta[name="publishdate"]`).prop(`content`)),8),t.author=l(`meta[name="author"]`).prop(`content`)||l(`meta[name="source"]`).prop(`content`),t.content={html:d,text:l(`div.content_body_box`).text()},t.language=h,t.enclosure_url=l(`div.content_body_box video`).prop(`src`)??void 0,t.enclosure_type=t.enclosure_url?`video/mp4`:void 0,t.enclosure_title=t.enclosure_url?u:void 0,t})));let _=new URL(m(`div.logo img`).prop(`src`),d).href;return{title:m(`title`).text(),link:f,item:g,allowEmpty:!0,image:_,language:h}},u={path:`/:category{.+}?`,name:`分类`,url:`www.agri.cn`,maintainers:[`nczitzk`],handler:l,example:`/agri/zx/zxfb`,parameters:{category:"分类，默认为 `zx/zxfb`，即最新发布，可在对应分类页 URL 中找到"},description:`::: tip
